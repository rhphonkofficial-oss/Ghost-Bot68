							...
							if (err) {
								...
								if (members.find(item => item.userID == uid)?.inGroup) // check if user is still in group
									...
									return message.reply(getLang("noPermission5"), (e, info) => {
										const { onEvent } = global.Ghost Bot;
										onEvent.push({
												...
												if (event.logMessageType === "log:thread-admins" && event.logMessageData.ADMIN_EVENT == "add_admin") {
													...
													if (TARGET_ID == api.getCurrentUserID()) {
														...
														if ((warnList.find(user => user.uid == uid)?.list.length ?? 0) <= 3)
															global.Ghost Bot.onEvent = onEvent.filter(item => item.messageID != info.messageID);
														else
															api.removeUserFromGroup(uid, event.threadID, () => global.Ghost Bot.onEvent = onEvent.filter(item => item.messageID != info.messageID));
													}
												}
											}
										});
