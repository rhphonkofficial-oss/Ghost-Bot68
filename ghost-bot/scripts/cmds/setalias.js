	...
	onStart: async function ({ message, event, args, threadsData, globalData, role, getLang }) {
		...
		switch (args[0]) {
			case "add": {
				...
				if (!global.Ghost Bot.commands.has(commandName))
				...
				if (args[3] == '-g') {
					if (role > 1) {
						...
						if (global.Ghost Bot.aliases.has(alias))
							return message.reply(getLang("aliasExist", alias, global.Ghost Bot.aliases.get(alias)));
						...
						global.Ghost Bot.aliases.set(alias, commandName);
				...
				if (global.Ghost Bot.commands.get(alias))
			...
			case "rm": {
				...
				if (args[3] == '-g') {
					if (role > 1) {
						...
						await globalData.set('setalias', globalAliasesData, 'data');
						global.Ghost Bot.aliases.delete(alias);
